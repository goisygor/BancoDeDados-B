...
if (isset($_GET['id'])) {
    ...
    if (isset($_GET['confirm'])) {
        if ($_GET['confirm'] == 'yes') {
            ...
            $stmt = $pdo->prepare('DELETE FROM contatos WHERE id_completo = ?');
            $stmt->execute([$_GET['id']]);
...
<div class="content delete">
    ...
    <div class="yesno">
        <a href="delete.php?id=<?=$contact['id_completo']?>&confirm=yes">Sim</a>
        <a href="delete.php?id=<?=$contact['id_completo']?>&confirm=no">Não</a>
    </div>
    <?php endif; ?>
</div>
