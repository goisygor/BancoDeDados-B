...
if (isset($_GET['id'])) {
    if (!empty($_POST)) {
        ...
        $id_completo = isset($_POST['id']) ? $_POST['id'] : NULL;
        ...
        // Atualiza o registro
        $stmt = $pdo->prepare('UPDATE contatos SET id_completo = ?, nome = ?, email = ?, cel = ?, pizza = ?, cadastro = ? WHERE id_completo = ?');
        $stmt->execute([$id_completo, $nome, $email, $cel, $pizza, $cadastro, $_GET['id']]);
        $msg = 'Atualização Realizada com Sucesso!';
    }
    // Obter o contato da tabela contatoss
    $stmt = $pdo->prepare('SELECT * FROM contatos WHERE id_completo = ?');
...
<div class="content update">
    ...
    <form action="update.php?id=<?=$contatos['id_completo']?>" method="post">
        <label for="id_completo">ID</label>
        <label for="nome">Nome</label>
        <input type="text" name="id" placeholder="" value="<?=$contatos['id_completo']?>" id="id_completo">
